...
class ShizukuFileSystem {
    ...
    fun configPresetWriteStatus(state: ShizukuState): String =
        if (state == ShizukuState.READY) {
            "Safe-only gated; Balanced preview only"
        } else {
            "locked"
        }
    ...
    fun disabledReason(state: ShizukuState): String {
        ...
        return "Patch writing is available only through the PAK-only verified install flow. Config preset writing is available only through the Safe / Default verified flow; Balanced is preview-only."
    }
}
